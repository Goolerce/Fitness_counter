...
def video_process(video_path, flag, class_names):
    # 指定视频路径和输出名称
    # video_path = 'pushup-sample.mp4'
    # class_name需要与你的训练样本的两个动作状态图像文件夹的名字中的一个（或者是与fitness_poses_csvs_out中的一个csv文件的名字）
    # 保持一致，它后面将用于分类时的索引。
    ...
    if flag == 1:
        ...
        class_name = ['pull-up']
        out_video_path = 'pullup-sample-out.mp4'
    elif flag == 4:
        class_name = ['crunches-up']
        out_video_path = 'crunches-sample-out.mp4'
    ...
    with tqdm.tqdm(total=video_n_frames, position=0, leave=True) as pbar:
        while True:
            ...
            pose_3D = pose_landmarks
            # for i in range(33):
            #     pose_3D.landmark[0].z *= 4
