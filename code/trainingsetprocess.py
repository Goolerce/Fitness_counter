...
def trainset_process(flag):
    ...
    if flag == 1:
        if os.path.isfile('fitness_poses_csvs_out/push_up.csv') and os.path.isfile('fitness_poses_csvs_out/push_down.csv'):
            return ['push_up.csv', 'push_down.csv']
    # 如果fitness_poses_csvs_out文件夹下的squat_down.csv和squat_up.csv已经存在，则不用导入样本图片再训练了
        ...
        if os.path.isfile('fitness_poses_csvs_out/squat_up.csv') and os.path.isfile('fitness_poses_csvs_out/squat_down.csv'):
            return ['squat_up.csv', 'squat_down.csv']
    # 如果fitness_poses_csvs_out文件夹下的pull_down.csv和pull_up.csv已经存在，则不用导入样本图片再训练了
    elif flag == 3:
        if os.path.isfile('fitness_poses_csvs_out/pull-up.csv') and os.path.isfile('fitness_poses_csvs_out/pull-down.csv'):
            return ['pull-up.csv', 'pull-down.csv']
    elif flag ==4:
        if os.path.isfile('fitness_poses_csvs_out/crunches-up.csv') and os.path.isfile('fitness_poses_csvs_out/crunches-down.csv'):
            return ['crunches-up.csv', 'crunches-down.csv']
    ...
    class_names = sorted([n for n in os.listdir(bootstrap_images_in_folder) if not n.startswith('.')])
    ...
    pose_classifier = pc.PoseClassifier(
        ...
        pose_class_names=[class_name + '.csv' for class_name in class_names],
...
# dump_for_the_app()
    ...
    return [class_name + '.csv' for class_name in class_names]